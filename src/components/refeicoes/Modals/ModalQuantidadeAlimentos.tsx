    ...
    if (showModalQtd == false) {
        ...
        return(
            <section className="fixed bottom-0 left-0 right-0 z-50 flex flex-col p-6 bg-nav-bg rounded-t-3xl border-t-2 border-bordeaux/20 shadow-2xl gap-4 max-w-md mx-auto">
                <div className="flex justify-between items-center pb-2 border-b border-bordeaux/10">
                    <button className="bg-transparent! p-0! text-bordeaux font-bold text-base hover:text-bordeaux cursor-pointer"
                     onClick={() => {
                        setShowModalQtd(false)
                        setQuantidade(1)
                    }}>Fechar</button>

                    <button type="button" className="px-4 py-1.5 font-bold text-sm text-bg cursor-pointer"
                     onClick={() => {
                        setAlimentosAdicionados([...alimentosAdicionados, {
                    ...
                    <h3 className="text-lg font-bold text-bordeaux text-center">{alimentoSelecionado?.name}</h3>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 text-center bg-rosa/20 p-3 rounded-2xl border border-bordeaux/20">
